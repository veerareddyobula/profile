...
export default props => {
  ...
  React.useEffect(() => {
    ...
    if (state) {
      params = { ...state, isEdit: true, tagList: state.tags.split(",") };
      ...
      params = {
        ...tempState,
        isEdit: true,
        tagList: tempState.tags.split(",")
      };
      history.replace({ pathname: location.pathname, state: tempState });
    }
  ...
  React.useEffect(() => {
    ...
    if (status && status.type === "ASYNC_REDIRECT_UPDATE_SUCCESS") {
      ...
      history.push({
        pathname: `/products/notes/admin/home`,
        state: { ...status }
      });
    }
  }, [asyncStore]);
      ...
      {initValues && (
        <Formik
          ...
          validate={values => {
            ...
            if (!values.uid) {
              errors.uid =
                "Please enter YouTube post id (copy from the YouTube URL ex: www.youtube.com/watch?v=<b>Y6aYx_KKM7A</b>) ";
            }
                ...
                <div className="col s12 m12 l6 border-left">
                  <div className="m-1">
                    ...
                    <div className="d-flex flex-wrap m-1">
                      <div className="m-1">
                        {values.tagList &&
                          values.tagList.map((item, index) => {
                            return (
                              <div
                                key={`tagListEntity${index}`}
                                className="chip"
                              >
                                {item}
                                <i
                                  ...
                                  onClick={() =>
                                    removeTag(values, item, setValues)
                                  }
                                >
                                  close
                                </i>
